...
async function completedSession(student_id, session_id) {
	...
	for (let i = 0; i < enroll_data_list.length; i++) {
		...
		if (enroll_data !== null) {
			...
			//TODO AUTO FINISHER
			const enrolldata_id = enroll_data.id;
			if (
				enroll_data.type === QUIZ &&
				scoringFunctions.isQuizLate(enrolldata_id)
			) {
				scoringFunctions.autoFinishQuizWithZero(enrolldata_id);
				status = FAILED;
			}
			// FISIH TODO AUTO FINISHER
		}
