...
exports.autoFinishQuizWithZero = async (material_enrolled_id) => {
	let result = 0;

	return result;
};

async function submitQuizAnswer(material_enrolled_id, answer) {
	let result = 0;
	return result;
}

async function gradeQuiz(student_answer, quiz_answer) {
	let result = {};

	const answer_length = quiz_answer.length;
	if (student_answer.length !== answer_length) {
		return null;
	}

	let correct_total = 0;
	for (let i; i < answer_length; i++) {
		if (student_answer[i] === quiz_answer[i]) {
			correct_total++;
		}
	}

	var score = correct_total / answer_length;
	result = {
		total_questions: answer_length,
		total_correct_answers: correct_total,
		score: score,
	};
	return result;
}
